	...
	do
	{
		...
		if(gp_selfos_cur_task->state==OS_SLEEP)
		{
			...
			if(pr->link.next==&(pr->link))	//已经是当前优先级的最后一个了
			{
				...
				((struct slefos_prio_struct *)pr_tmp)->prio=(size_t)NULL;
...
{
	return *((uint32_t *)((size_t)pr_struct+para_offset));
}
						...
						uint32_t para_offset,uint8_t order)
{
	...
	inser_node= (void *)((size_t)inser_list - list_offset);
	head_node= (void *)((size_t)head_list - list_offset);
	
	para_node = *((uint32_t *)((size_t)head_node+para_offset));
	para_new_node = *((uint32_t *)((size_t)inser_node+para_offset));
	...
	else
	{
		...
		do
		{
			cur_node = (void *)((size_t)pr_list - list_offset) ;	
			para_node = *((uint32_t *)((size_t)cur_node+para_offset));
			para_new_node = *((uint32_t *)((size_t)inser_node+para_offset));
